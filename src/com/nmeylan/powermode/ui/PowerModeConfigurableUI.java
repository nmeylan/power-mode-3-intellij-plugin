...
public class PowerModeConfigurableUI implements ConfigurableUi<PowerMode> {
  ...
  public PowerModeConfigurableUI(PowerMode powerMode) {
    ...
    bindSliders(sparkColorRedFrom, sparkColorRedTo);
    ...
    bindSliders(sparkColorGreenFrom, sparkColorGreenTo);
    ...
    bindSliders(sparkColorBlueFrom, sparkColorBlueTo);
    ...
    Component scrollPane = Arrays.asList(mainPanel.getComponents()).stream().filter(c -> c instanceof JScrollPane).findFirst().orElse(null);
    if (scrollPane != null) {
      ((JScrollPane) scrollPane).getVerticalScrollBar().setUnitIncrement(16);
    }
    {
  ...
  private void bindSliders(JSlider from, JSlider to) {
